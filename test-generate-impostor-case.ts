...
async function testGenerateImpostorCase() {
  ...
  const testData = {
    ...
    language: 'es',
    playerNames: ['Ana', 'Carlos', 'María', 'Luis'],
    playerGenders: ['female', 'male', 'female', 'male']
  }
  ...
  try {
    ...
    data.players.forEach((player: any, index: number) => {
      const isKiller = player.id === data.hiddenContext.killerId
      console.log(`   ${index + 1}. ${player.name} (${player.role}, ${player.age} años) ${isKiller ? '🔴 [ASESINO]' : ''} - ${player.isKiller ? 'Es asesino' : 'No es asesino'}`)
    })
